...
const useSmartAppBannerCallbacks = (appId: ComputedRef<string>, platform: Ref<SmartAppBannerPlatform>, setShowBanner: (value: boolean) => void, emit: any, bannerConfig: SmartBannerOptions) => {
  const installClick = () => {
    setShowBanner(false)
    if (bannerConfig.daysReminder > 0) {
    ...
    emit('onClose', platform.value, appId.value)
  }

  const dismissClick = () => {
    setShowBanner(false)
    if (bannerConfig.daysHidden > 0) {
...
export const useSmartAppBanner = (emit: any) => {
  const bannerConfig = inject('smartAppBannerConfig') as SmartBannerOptions
  const {
    appId,
    inStoreText,
    platform,
